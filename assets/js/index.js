...
stevenBtn.addEventListener("click", runQuiz);
timBtn.addEventListener("click", runQuiz);
alfredBtn.addEventListener("click", runQuiz);

function runQuiz(event) {
  chooseDirector = event.target.value;
  if(chooseDirector === "stevenBtn") {
    shuffledQuestions = quizSteven.sort(() => Math.random() * availableQuesions.length);
    currentQuestion = shuffledQuestions;
  } else if (chooseDirector === "timBtn") {
    shuffledQuestions = quizTim.sort(() => Math.random() * availableQuesions.length);
    currentQuestion = shuffledQuestions;
  } else if (chooseDirector === "alfredBtn"){
    shuffledQuestions = quizAlfred.sort(() => Math.random() * availableQuesions.length);
    currentQuestion = shuffledQuestions;
  }
}
...
// Add function for 
  ...
  questionCounter = 0;
  score = 0;
  availableQuesions = [...quizAlfred, ...quizSteven, ...quizTim];
  getNewQuestion();
};
